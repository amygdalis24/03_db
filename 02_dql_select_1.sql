...
select
    menu_name as 메뉴명,
    menu_price as "메뉴 가격",   # 쌍따옴표를 이용해서 띄어쓰기된 컬럼명을 묶기 가능
...
select
    ...
    10 / 3,
    10 % 3,
    10 div 3,   # python의 // 연산과 같음
    10 mod 3   # modulo == % 연산과 같음
...
select
    menu_name as 메뉴명,
    concat(menu_price, '원') as '메뉴 가격(원)'   # -> 작은 따옴표가 문자열
from
    tbl_menu;
...
# 기준으로 삼은 컬러밍 여러 개 존재하면 그룹화된 정렬이 수행
# 메뉴 테이블에서
# 메뉴명, 카테고리코드, 가격을 조회
# 단, 카테고리 코드 오름차순, 가격 내림차순으로 조회
# 1) 카테고리 코드 오름차순 정렬을 먼저 수행
...
from
    tbl_category
where
    # ref_category_code 컬럼 안에 null 이란 값이 있으면 True
    # -> null은 값(data)가 아님. 그냥 컬럼이 비었음!!!
    # ref_category_code = null;   # 모든 행 false

    # ref_cotegory_code 칼럼에 값이 비어있으면 True
    # ref_category_code is null;

    # ref_cotegory_code 칼럼에 값이 비어있지 않으면 True
    ref_category_code is not null;

# ================================================
# limit
# - 조회 결과(ResultSet)에서 지정된 크기만큼의 행만 조회
# limit n : 0번 인덱스(1행) 부터 n행만큼 조회
# limit start, n : start 인덱스부터 n행 만큼
# 5~10 인덱스가 아니라 5행부터 10개의 행을 출력
# limit offset, n : offset 행 만큼 건너뛰고 n행만큼 조회
