...
# - 데이터베이스 스키마(객체)를
#   만들고(CREATE), 수정하고(UPDATE: 전체수정, MODIFY: 부분 수정, ALTER: 바꾸다(구조 수정)), 삭제하는 구문
...
# product 테이블에 데이터를 추가(INSERT)
insert into product(name) values('텀블러');
insert into product(name, price) values('머그컴', 5000);
...
# Duplicate entry '010-777-7777' for key 'suer_unique.phone'
...
# - NOT NULL + UNIQUE 특징을 가짐 (중복 되지 않는 값 필수)
...
# - 참조는 다른 테이블의 PK 또는 UNIQUE가 설정된 컬럼만 가능하다
